...
### 2026-06-08 V6.0 RBAC 细粒度权限控制系统的实现
- **权限门控组件**：`PermissionGate.tsx` 支持 `codename` / `anyCodenames` 两种模式，`hide` / `disabled` / `forbidden` 三种 fallback 行为
- **权限状态管理**：`permission-store.ts` — Zustand 管理权限列表 + `hasPermission / hasAnyPermission / hasAllPermissions` 检查 + 超户豁免
- **权限 Hook**：`hooks/usePermissions.ts` — 组件挂载时自动加载权限
- **Sidebar 权限过滤**：菜单项按 `MENU_PERMISSIONS` 映射动态显隐，连续分隔符合并
- **全页面接入**：~80 个前端页面通过 `PermissionGate` 包裹实现权限控制
- **权限生命周期**：`auth-store` 中登录/注册后自动 `fetchPermissions()`，登出时 `reset()`
- **国际化 metadata**：`layout.tsx` `generateMetadata` 支持根据 locale 动态获取描述
- **后端权限修正**：`performance/api.py` 权限修饰符从 `test_mgmt.*` 统一为 `performance.*`
- **后端测试**：新增 `tests/test_auth/` 和 `tests/test_rbac/` 测试套件
- **国际化消息**：`zh-cn.json` / `en.json` 新增 ~1500 行权限 & 页面相关消息
