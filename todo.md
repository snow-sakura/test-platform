...
## 当前版本：V5.0 AI 智能模式 + 数据工厂 + AI 评测师

### V5.0 范围
- AI 智能模式（browser-use 代理执行、执行记录、报告生成、PDF 导出）
- 数据工厂（场景数据生成、变量函数助手、记录管理）
- AI 评测师（Dify 配置管理、会话对话、SSE 流式聊天）
- AI 用例生成（需求文档上传/分析、LLM 用例生成、SSE 进度推送、配置中心）

### V5.0 已完成内容
- [x] AI 智能模式 — 执行管理、用例管理、执行记录、报告详情 + PDF 导出
- [x] 数据工厂 — 场景数据生成、工具/记录/场景视图、变量函数助手
- [x] AI 评测师 — Dify 配置 CRUD + 测试连接、会话管理、流式/非流式聊天
- [x] AI 用例生成 — 8 个数据模型、24 个 API 端点、LLM 生成工作流、SSE 进度推送、配置中心
- [x] 项目成员管理 — 成员模型、角色权限、前后端完整 CRUD
- [x] 全量需求文档审计 — 15 个模块 vs 18 个文档，补齐所有缺口

### V6.0 规划方向

| 方向 | 说明 | 优先级 |
|------|------|--------|
| RBAC 细粒度权限系统 | 全局角色 + 权限点 + 资源级授权 | P0 |
| 国际化全面覆盖 | 消除硬编码中文标签，统一走 next-intl | P0 |
| Docker 部署 | Docker Compose + 一键启动脚本 | P1 |
| 性能测试模块 | JMeter 集成 + 压测场景编排 + 报告可视化 | P2 |
| CI/CD 集成 | Jenkins/GitLab 插件 + Webhook 触发测试 | P2 |
| 移动端适配 | 响应式布局、触摸优化 | P3 |
