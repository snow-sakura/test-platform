...
### 范围
- 完整接口测试管理平台，包含 9 个数据模型、20+ API 端点、9 个前端页面
- API 项目 CRUD + 集合树形结构（无限级嵌套）
- 接口请求管理（HTTP 方法/URL/Headers/Body/Params）
- 真实 HTTP 请求执行（httpx.AsyncClient，30s 超时）
- 环境变量管理（Global/Local 作用域，${var} 模板变量解析）
- 测试套件（有序请求列表 + 批量执行 + 断言）
- 请求历史（分页/搜索/筛选/详情）
- 定时任务（Cron 表达式 + 暂停/恢复/立即执行）
- 通知管理（飞书/企微/钉钉 Webhook + 签名 + 发送日志）
- 仪表盘统计（项目数/接口数/套件数/今日执行）
...
#### 后端（9 个数据模型 + 20+ API 端点 + 3 个核心服务）
- **数据模型**：ApiProject/ApiCollection（树形自引用）/ApiRequest（核心模型）/ApiTestSuite/ApiEnvironment/ApiRequestHistory/ApiScheduledTask/ApiNotificationConfig/ApiNotificationLog
- **API 路由**：仪表盘 / 项目 CRUD / 集合树 / 请求 CRUD + 执行 / 套件 CRUD + 执行 / 环境 CRUD + 激活 / 历史分页 + 详情 / 定时任务 CRUD + 暂停恢复 / 通知 CRUD + 测试发送 / 通知日志
- **VariableResolver**：正则 `${var_name}` 模板变量解析，支持 random_str/random_int/timestamp/uuid 内置函数
- **RequestExecutor**：httpx.AsyncClient 30s 超时，支持 JSON/FormData/FormUrlencoded/none body 类型
- **NotificationSender**：飞书（HMAC-SHA256 签名）/企微/钉钉 Webhook 异步发送
- **APScheduler 集成**：`core/scheduler.py` 配合 `modules/api_testing/scheduler.py` 启动时自动加载 active 任务

#### 前端（9 个页面 + 5 个共享组件）
- **共享组件**：KeyValueEditor / BodyEditor / ResponseViewer / CollectionTree / RequestEditor / ApiProjectFormModal
- **仪表盘**：4 个 Statistic 统计卡片（项目数/接口数/套件数/今日执行）
- **API 项目页**：CRUD 表格（类型/状态 Tag，集合数/请求数列）
- **接口管理页**（核心）：左树右编辑器布局，方法选择器/URL 输入/Params/Headers/Body Tabs + 响应面板
- **自动化页**：套件列表 + 请求列表 + 执行按钮 + 结果汇总
- **环境管理页**：Global/Local Tab，KeyValue 变量编辑，激活按钮
- **请求历史页**：搜索/筛选/批量删除/清空 + 详情弹窗
- **定时任务页**：CRON 管理 + 暂停/恢复/立即执行
- **通知管理页**：通知配置 CRUD + 测试发送 + 发送日志双 Tab
- **侧边栏**：接口测试菜单已激活（`/api-testing` 路由）

#### 迁移 & 验证
- **Alembic 迁移**：`f3634490698b_add_api_testing_models` 自动生成并应用（9 张新表）
- **后端验证**：20+ API 端点全部返回 200/201，真实 HTTP 请求执行成功（jsonplaceholder）
- **前端验证**：9 个页面全部编译通过并返回 200
- **Bug 修复**：Pydantic v2 `@field_serializer` → `@field_validator(mode='before')` datetime 类型转换

#### 文件变更统计
- **后端创建**：7 个文件（`__init__.py`/`models.py`/`schemas.py`/`crud.py`/`services.py`/`api.py`/`scheduler.py`）
- **后端修改**：3 个文件（`main.py`/`alembic/env.py`/`requirements.txt`）
- **前端创建**：~15 个文件（5 个组件 + 9 个页面/layout + 1 个 API 层）
- **前端修改**：1 个文件（`Sidebar.tsx`）
- **迁移**：1 个 Alembic 脚本
- **总计**：约 25 个文件新增/修改
